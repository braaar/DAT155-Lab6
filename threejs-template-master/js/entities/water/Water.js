import {
    ClampToEdgeWrapping, DataTexture,
    FloatType,
    HalfFloatType, LinearFilter, Mesh, NearestFilter,
    OrthographicCamera, PlaneBufferGeometry, RepeatWrapping, RGBAFormat,
    Scene, ShaderMaterial, TextureLoader, UniformsUtils, Vector2, Vector3, WebGLRenderTarget
...
let Water = function ( renderer, camera, scene, options ) {
    ...
    let renderTargetType = optionalParameter( options.USE_HALF_FLOAT, false ) ? HalfFloatType : FloatType;
    let LinearClampParams = {
    ...
    let fullscreeenVertexShader = waterShader[ "ocean_sim_vertex" ];

    // 1 - Horizontal wave vertices used for FFT
    let oceanHorizontalShader = waterShader[ "ocean_subtransform" ];
    let oceanHorizontalUniforms = UniformsUtils.clone( oceanHorizontalShader.uniforms );
    ...
    let oceanVerticalShader = waterShader[ "ocean_subtransform" ];
    let oceanVerticalUniforms = UniformsUtils.clone( oceanVerticalShader.uniforms );
    ...
    let initialSpectrumShader = waterShader[ "ocean_initial_spectrum" ];
    let initialSpectrumUniforms = UniformsUtils.clone( initialSpectrumShader.uniforms );
    ...
    let phaseShader = waterShader[ "ocean_phase" ];
    let phaseUniforms = UniformsUtils.clone( phaseShader.uniforms );
    ...
    let spectrumShader = waterShader[ "ocean_spectrum" ];
    let spectrumUniforms = UniformsUtils.clone( spectrumShader.uniforms );
    ...
    let normalShader = waterShader[ "ocean_normals" ];
    let normalUniforms = UniformsUtils.clone( normalShader.uniforms );
    ...
    let vannShader = waterShader[ "ocean_main" ];
    let oceanUniforms = UniformsUtils.clone( vannShader.uniforms );
    this.materialOcean = new ShaderMaterial( {
        uniforms: oceanUniforms,
        vertexShader: vannShader.vertexShader,
        fragmentShader: vannShader.fragmentShader
    } );
    ...
    this.screenQuad = new Mesh( new PlaneBufferGeometry( 2, 2 ), new TextureLoader().load('js/entities/water/vann2.jpg') );
...
Water.prototype.generateMesh = function () {
    ...
    let geometry = new PlaneBufferGeometry( this.geometrySize, this.geometrySize, this.geometryResolution, this.geometryResolution );
